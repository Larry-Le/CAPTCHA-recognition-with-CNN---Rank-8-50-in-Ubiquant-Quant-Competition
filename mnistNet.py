...
new_x_train = np.array(np.array([0]*x_train.shape[0]*72*72)).reshape(x_train.shape[0],72,72)
new_x_test = np.array(np.array([0]*x_test.shape[0]*72*72)).reshape(x_test.shape[0],72,72)
# for i in x_train:
for i in range(len(new_x_train)):
    new_x_train[i,:,:] = cv2.resize(x_train[i,:,:], (72, 72))
x_train = new_x_train

for i in range(len(new_x_test)):
    new_x_test[i,:,:] = cv2.resize(x_test[i,:,:], (72, 72))
x_test = new_x_test
...
for i in range(20):
        ...
        model=make_model()
        # else:
        #     model = tf.keras.models.load_model("model_save/model_{}.h5".format(i-1))
        ...
        model.save("model_save/model_" + str(i) + ".h5")
